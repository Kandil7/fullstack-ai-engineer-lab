...
def create_department_with_professors(
    db: Session, department: DepartmentCreate, professors: List[ProfessorCreate]
):
    # Create department
    ...
    for prof in professors:
        db_professor = Professor(name=prof.name, department_id=db_department.id)
        db.add(db_professor)
...
def get_course_with_details(db: Session, course_id: int):
    stmt = (
        ...
        .options(
            ...
            joinedload(Course.enrollments).selectinload(Enrollment.student),
        )
...
def update_student_courses(db: Session, student_id: int, course_ids: List[int]):
...
def search_courses(
    ...
    if query:
        filters.append(
            or_(Course.title.ilike(f"%{query}%"), Course.description.ilike(f"%{query}%"))
        )
...
def get_students_above_average(db: Session, course_id: int):
    ...
    stmt = (
        select(Student)
        .join(Enrollment)
        .where(and_(Enrollment.course_id == course_id, Enrollment.grade > avg_subquery))
    )
...
# GOOD: Eager loading
departments = db.query(Department).options(joinedload(Department.professors)).all()
```
