...
async def gather_with_timeout():
    results = await asyncio.wait_for(asyncio.gather(fetch_user(1), fetch_user(2)), timeout=5.0)
    return results
```
...
async def use_thread_executor():
    loop = asyncio.get_event_loop()
    result = await loop.run_in_executor(thread_executor, blocking_io_operation)
    return result
...
async def use_process_executor():
    loop = asyncio.get_event_loop()
    result = await loop.run_in_executor(process_executor, cpu_intensive_operation)
    return result
...
async def protected_operation():
    try:
        result = await asyncio.wait_for(asyncio.shield(important_operation()), timeout=5.0)
    except asyncio.TimeoutError:
...
async def pipeline():
    ...
    await asyncio.gather(producer(q1), stage1(q1, q2), stage2(q2, q3), consumer(q3))
```
