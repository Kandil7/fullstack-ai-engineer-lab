...
class SafeStore:
    ...
    def __init__(self) -> None:
        ...
        self._conn.execute("CREATE TABLE users (id TEXT, name TEXT)")

    def add(self, user_id: str, name: str) -> None:
        """Insert a user. O(1)."""
        self._conn.execute("INSERT INTO users VALUES (?, ?)", (user_id, name))
        self._conn.commit()

    def find(self, user_id: str) -> list[tuple]:
        """Parameterized SELECT; injection payloads return []. O(1)."""
        cursor = self._conn.execute("SELECT name FROM users WHERE id = ?", (user_id,))
...
def load_config(path: Path) -> dict:
    ...
    for key, expected in TYPES.items():
        if key in raw and not isinstance(raw[key], expected):
            raise ValueError(f"{key} must be {expected.__name__}, got {type(raw[key]).__name__}")
    return raw
