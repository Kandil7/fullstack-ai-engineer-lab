...
class DataTransformPlugin(Plugin):
    ...
    def execute(self, context: dict) -> dict:
        data = context.get("data", [])
        transformed = [
            {k: v.upper() if isinstance(v, str) else v for k, v in d.items()} for d in data
        ]
...
class Pipeline:
    def __init__(self, source: DataSource, transformers: list[DataTransformer], sink: DataSink):
        self.source = source
