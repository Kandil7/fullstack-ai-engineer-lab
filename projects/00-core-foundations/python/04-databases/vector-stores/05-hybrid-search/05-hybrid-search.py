...
for qi in (7, 8):
    ...
    print(
        f"  q{qi} {queries[qi]!r:24s} bm25={b} dense={d} {'AGREE' if b[0] == d[0] else 'DISAGREE'}"
    )
...
for qid in (0, 3):
    fused = rrf_fusion([sparse_topk(queries[qid], k=8), dense_topk(q_emb[qid], k=8)])
...
def _verify() -> None:
    ...
    assert np.isclose(minmax(raw_sparse).max(), 1.0) and np.isclose(minmax(raw_dense).max(), 1.0), (
    ...
    intended = {0: {6}, 1: {1}, 2: {11}, 3: {10, 1}, 4: {0}, 5: {3}, 6: {4}, 7: {9, 10}, 8: {10, 6}}
    for i, hits in enumerate(rrf_hits):
        assert intended[i] & set(hits), f"RRF top-3 for q{i} must contain an intended doc"
    ...
    assert r50 >= r0 and r50 >= r100, "blended alpha must be at least as good as either pure arm"

    # tenant filter: results must respect the tenant
    for qid in (0, 3):
        fused = rrf_fusion([sparse_topk(queries[qid], k=8), dense_topk(q_emb[qid], k=8)])
        in_b = [d for d in fused if tenants[d] == "b"][:3]
        assert all(tenants[d] == "b" for d in in_b), "tenant filter must isolate b"
