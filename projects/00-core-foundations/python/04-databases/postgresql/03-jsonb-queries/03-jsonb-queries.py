...
tags = conn.execute("SELECT json_extract(meta, '$.tags') FROM model_cards").fetchone()[0]
kind = conn.execute("SELECT json_type(meta, '$.tags') FROM model_cards").fetchone()[0]
...
missing = conn.execute("SELECT json_extract(meta, '$.nope') FROM model_cards").fetchone()[0]
present_null = conn.execute("SELECT json_type(meta, '$.owner') FROM model_cards").fetchone()[0]
...
conn.execute("CREATE INDEX idx_cards_model ON model_cards (json_extract(meta, '$.model'))")
    ...
    try:
        with psycopg.connect(dsn, connect_timeout=1) as pg:
            with pg.cursor() as cur:
                ...
                cur.execute("CREATE INDEX ON cards USING gin (meta)")
...
def _verify() -> None:
    ...
    with sqlite3.connect(":memory:") as conn:
        ...
        conn.execute("CREATE INDEX idx_c_model ON c (json_extract(meta, '$.model'))")
        plan = conn.execute(
        ...
        assert any("idx_n_model" in str(row) for row in plan), "normalized index must be used"
