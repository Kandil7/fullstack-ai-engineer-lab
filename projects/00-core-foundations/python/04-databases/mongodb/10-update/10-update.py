...
result = update_one(
    users,
    {"name": "Alice"},
    {"$inc": {"age": 1}, "$set": {"status": "active"}, "$push": {"scores": 100}},
)
...
def _verify() -> None:
    ...
    assert u1[0]["age"] == 26
    assert (
        update_one(u1, {"name": "Bob"}, {"$set": {"age": 31, "email": "bob_new@mail.com"}})[
            "modified_count"
        ]
        == 1
    )
    ...
    u5 = reset_users()
    assert (
        update_many(u5, {"status": "active"}, {"$set": {"verified": True}})["modified_count"] == 3
    )
    ...
    assert (
        update_nested(users_with_address, {"name": "Alice"}, "address.city", "Los Angeles")[
            "modified_count"
        ]
        == 1
    )
