...
def filter_range(conn: sqlite3.Connection, lo: float, hi: float) -> list[str]:
    ...
    return [
        ...
        for r in conn.execute(
            "SELECT name FROM models WHERE metric >= ? AND metric <= ? ORDER BY name", (lo, hi)
        )
    ]
...
def pattern_match(conn: sqlite3.Connection, pattern: str) -> dict:
    ...
    names = [
        r[0]
        for r in conn.execute("SELECT name FROM models WHERE name LIKE ? ORDER BY name", (pattern,))
    ]
