...
def top_n(conn: sqlite3.Connection, n: int) -> list[tuple]:
    ...
    return [
        ...
        for r in conn.execute(
            "SELECT name, metric FROM models ORDER BY metric DESC, name ASC LIMIT ?", (n,)
        )
    ]
...
def metric_report(conn: sqlite3.Connection) -> dict:
    ...
    report = [
        tuple(r)
        for r in conn.execute(
            "SELECT DISTINCT name, metric * 100 AS score FROM models ORDER BY score DESC"
        )
    ]
    distinct = conn.execute("SELECT COUNT(DISTINCT name) FROM models").fetchone()[0]
...
def paginate(conn: sqlite3.Connection, page_size: int, page: int) -> dict:
    ...
    rows = [
        tuple(r)
        for r in conn.execute(
            "SELECT name, metric FROM models ORDER BY metric DESC, name ASC LIMIT ? OFFSET ?",
            (page_size, offset),
        )
    ]
