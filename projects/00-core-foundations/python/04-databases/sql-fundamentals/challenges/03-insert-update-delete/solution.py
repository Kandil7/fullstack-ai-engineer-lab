...
def insert_models(conn: sqlite3.Connection, rows: list[tuple]) -> int:
    ...
    cur = conn.executemany("INSERT INTO models (name, epoch, metric) VALUES (?, ?, ?)", rows)
    return cur.rowcount
...
def sync_models(conn: sqlite3.Connection, rows: list[tuple]) -> list[tuple]:
    ...
    return [tuple(r) for r in conn.execute("SELECT name, epoch FROM models ORDER BY name")]
...
def apply_changeset(conn: sqlite3.Connection, ops: list[tuple]) -> list[int]:
    ...
    for op in ops:
        ...
        if kind == "insert":
            _, name, epoch = op
            row = conn.execute(
                "INSERT INTO models (name, epoch, metric) VALUES (?, ?, ?) RETURNING id",
                (name, epoch, 0.0),
            ).fetchone()
            ids.append(row[0])
        elif kind == "update":
            _, name, epoch = op
            row = conn.execute(
                "UPDATE models SET epoch = ? WHERE name = ? RETURNING id", (epoch, name)
            ).fetchone()
            if row is not None:
                ids.append(row[0])
        elif kind == "delete":
            _, name = op
            row = conn.execute("DELETE FROM models WHERE name = ? RETURNING id", (name,)).fetchone()
            if row is not None:
                ids.append(row[0])
    return ids
