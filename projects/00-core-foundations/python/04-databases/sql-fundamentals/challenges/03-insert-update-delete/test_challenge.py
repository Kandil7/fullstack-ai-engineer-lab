...
class TestInsertModels:
    ...
    def test_reuses_existing_table(self):
        conn = fresh_conn()
        conn.execute(
            "CREATE TABLE models (id INTEGER PRIMARY KEY, name TEXT UNIQUE, epoch INT, metric REAL)"
        )
        conn.execute("INSERT INTO models (name, epoch, metric) VALUES (?, ?, ?)", ("old", 0, 0.0))
...
class TestSyncModels:
    ...
    def test_sorted_by_name(self):
        conn = fresh_conn()
        result = solution_module.sync_models(conn, [("zebra", 1, 0.1), ("alpha", 2, 0.2)])
...
class TestApplyChangeset:
    def test_ids_in_operation_order(self):
        conn = fresh_conn()
        ids = solution_module.apply_changeset(
            conn,
            [
                ("insert", "a", 1),
                ("insert", "b", 1),
    ...
    def test_final_state_correct(self):
        conn = fresh_conn()
        solution_module.apply_changeset(
            conn,
            [
                ("insert", "a", 1),
                ("insert", "b", 2),
