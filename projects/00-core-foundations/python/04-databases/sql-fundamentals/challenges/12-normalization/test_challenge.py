...
class TestSplitCsvColumn:
    def test_splits_csv(self):
        ...
        conn.executemany(
            "INSERT INTO contacts (id, email, tags_csv) VALUES (?, ?, ?)",
...
class TestSplitDepartments:
    def test_distinct_departments(self):
        ...
        conn.executemany(
            ...
            [("ana", "eng", "floor1"), ("bob", "eng", "floor1"), ("cam", "ml", "floor2")],
        )
...
class TestBuildStarSchema:
    def _log(self):
        conn = sqlite3.connect(":memory:")
        conn.execute("CREATE TABLE events_log (date TEXT, user TEXT, product TEXT, amount REAL)")
        conn.executemany(
            "INSERT INTO events_log (date, user, product, amount) VALUES (?, ?, ?, ?)",
            [
                ("2026-08-01", "ana", "gpu", 10.0),
                ("2026-08-01", "ana", "cpu", 5.0),
