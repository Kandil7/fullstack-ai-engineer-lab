...
def split_csv_column(conn: sqlite3.Connection) -> list[tuple]:
    ...
    conn.execute("CREATE TABLE IF NOT EXISTS contact_tags (contact_id INTEGER, tag TEXT)")
    rows = []
    for cid, tags in conn.execute("SELECT id, tags_csv FROM contacts"):
        for tag in tags.split(","):
            ...
            if tag:
                rows.append((cid, tag))
    conn.executemany("INSERT INTO contact_tags (contact_id, tag) VALUES (?, ?)", rows)
    return [
        tuple(r)
        for r in conn.execute("SELECT contact_id, tag FROM contact_tags ORDER BY contact_id, tag")
    ]
...
def split_departments(conn: sqlite3.Connection) -> dict:
    ...
    conn.execute(
        "CREATE TABLE IF NOT EXISTS departments (dept_name TEXT PRIMARY KEY, location TEXT)"
    )
    conn.execute(
    ...
    depts = sorted(
        {(d, loc) for _, _, d, loc in conn.execute("SELECT * FROM employees")}, key=lambda x: x[0]
    )
    conn.executemany("INSERT INTO departments (dept_name, location) VALUES (?, ?)", depts)
    conn.execute(
        "INSERT INTO employees_normalized (name, dept_name) SELECT name, dept_name FROM employees"
    )
    ...
    return {
        ...
        "locations": [
            r[0]
            for r in conn.execute("SELECT DISTINCT location FROM departments ORDER BY location")
        ],
    }
