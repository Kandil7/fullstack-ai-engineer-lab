...
def sargable_plan(conn: sqlite3.Connection) -> dict:
    ...
    return {
        "sargable": [
            r[3]
            for r in conn.execute("EXPLAIN QUERY PLAN SELECT * FROM events WHERE ts >= ?", (2500,))
        ],
        "wrapped": [
            r[3]
...
def keyset_page(conn: sqlite3.Connection, after_id: int, limit: int) -> dict:
    ...
    conn.executemany("INSERT INTO events (name) VALUES (?)", [(f"e{i}",) for i in range(1, 5001)])
    rows = [
        tuple(r)
        for r in conn.execute(
            "SELECT id, name FROM events WHERE id > ? ORDER BY id LIMIT ?", (after_id, limit)
        )
    ]
    plan = [
        r[3]
        for r in conn.execute(
            "EXPLAIN QUERY PLAN SELECT id FROM events WHERE id > ? ORDER BY id LIMIT ?",
            (after_id, limit),
        )
    ]
