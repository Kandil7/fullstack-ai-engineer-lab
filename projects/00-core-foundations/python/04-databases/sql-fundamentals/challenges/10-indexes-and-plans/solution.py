...
def sargable_vs_not(conn: sqlite3.Connection) -> dict:
    ...
    conn.executemany("INSERT INTO events (ts) VALUES (?)", [(i,) for i in range(1, 5001)])
...
def covering_vs_table(conn: sqlite3.Connection) -> dict:
    ...
    return {
        "covering": plan_for(conn, "SELECT model, latency FROM events WHERE model = ?", ("m1",)),
        "star": plan_for(conn, "SELECT * FROM events WHERE model = ?", ("m1",)),
    }
...
def index_strategy(conn: sqlite3.Connection) -> dict:
    ...
    conn.executemany(
        ...
        [
            (f"m{i % 3}", round(i * 0.01, 3), 1700000000 + i, "active" if i % 2 else "archived")
            for i in range(1, 5001)
        ],
    )
    conn.execute(
        "CREATE INDEX idx_events_model_latency_created ON events(model, latency, created_at)"
    )
    conn.execute("CREATE INDEX idx_events_model_created ON events(model, created_at)")
    conn.execute("CREATE INDEX idx_events_active ON events(status) WHERE status = 'active'")
    return {
        ...
        "partial": plan_for(conn, "SELECT id FROM events WHERE status = 'active'", ()),
    }
