...
X, y = make_classification(
    n_samples=5000, n_features=20, n_informative=10, n_redundant=3, random_state=0
)
...
print(
    f"  AUC with categorical column handled natively: {roc_auc_score(cat_y, hgb_cat.predict_proba(cat_df)[:, 1]):.3f}"
)
...
rf = RandomForestClassifier(n_estimators=100, random_state=0).fit(
    Xr_tr, (yr_tr > yr_tr.mean()).astype(int)
)
