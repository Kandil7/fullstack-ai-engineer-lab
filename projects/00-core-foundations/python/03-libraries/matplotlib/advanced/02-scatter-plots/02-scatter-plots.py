...
sc3 = axes[1, 1].scatter(x, y, c=z, cmap="plasma", alpha=0.7, s=50, vmin=-2, vmax=2)  # Fixed range
...
axes[0, 0].plot(
    line_x, line_y, "r-", linewidth=2, label=f"y={slope:.2f}x+{intercept:.2f}, R²={r_value**2:.3f}"
)
axes[0, 0].legend()
...
hb2 = axes[0, 1].hexbin(x_large, y_large, gridsize=30, cmap="Reds", bins="log", mincnt=1)
...
# Quadrant labels
ax.text(0.95, 0.95, "Q1", transform=ax.transAxes, fontsize=14, ha="right", va="top", alpha=0.5)
ax.text(0.05, 0.95, "Q2", transform=ax.transAxes, fontsize=14, ha="left", va="top", alpha=0.5)
ax.text(0.05, 0.05, "Q3", transform=ax.transAxes, fontsize=14, ha="left", va="bottom", alpha=0.5)
ax.text(0.95, 0.05, "Q4", transform=ax.transAxes, fontsize=14, ha="right", va="bottom", alpha=0.5)
