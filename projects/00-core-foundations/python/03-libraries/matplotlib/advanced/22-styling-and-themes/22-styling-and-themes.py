...
def _verify() -> None:
    ...
    set_publication_rcparams()
    assert plt.rcParams["figure.dpi"] == 120, "rcParams assignment must take effect immediately"
    assert plt.rcParams["axes.grid"] is True, "rcParams grid default must be True after our setup"
    assert "ggplot" in plt.style.available, "ggplot must be a registered stylesheet"

    cmap = plt.get_cmap("viridis")
    assert cmap.name == "viridis", "get_cmap must return the named map"
    assert is_perceptually_uniform("viridis"), "viridis must be flagged perceptually uniform"
    assert not is_perceptually_uniform("jet"), "jet must be flagged as NOT perceptually uniform"
    assert mcolors.is_color_like("tab:blue"), "tab:blue must be a valid color spec"
