...
axes[0, 1].pie(sizes, explode=explode, labels=labels, autopct="%1.1f%%", colors=colors, shadow=True)
...
axes[1, 0].pie(
    sizes, labels=labels, autopct="%1.1f%%", colors=colors, startangle=90, counterclock=False
)
axes[1, 0].set_title("startangle=90, counterclock=False")

# Legend instead of labels
wedges, texts, autotexts = axes[1, 1].pie(
    sizes, autopct="%1.1f%%", colors=colors, labeldistance=1.15
)
axes[1, 1].legend(
    wedges, labels, title="Products", loc="center left", bbox_to_anchor=(1, 0, 0.5, 1)
)
...
for data, label, color in zip(
    [person_a, person_b, person_c], ["Person A", "Person B", "Person C"], ["blue", "red", "green"]
):
...
for angle, value, cat in zip(angles2[:-1], person_a[:-1], categories):
    axes[1].text(
        angle, value + 0.5, str(value), ha="center", va="center", fontweight="bold", color="navy"
    )
