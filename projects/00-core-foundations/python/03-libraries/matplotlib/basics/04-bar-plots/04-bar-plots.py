...
for i, (series, color) in enumerate(
    zip(["Series 1", "Series 2", "Series 3"], ["skyblue", "lightcoral", "lightgreen"])
):
    axes[1, 0].bar(
        bar_positions + (i - 1) * bar_width,
        df[series],
        bar_width,
        label=series,
        color=color,
...
for series, color in zip(
    ["Series 1", "Series 2", "Series 3"], ["skyblue", "lightcoral", "lightgreen"]
):
    axes[0, 0].bar(df["Category"], df[series], bottom=bottom, label=series, color=color)
...
for series, color in zip(
    ...
    axes[0, 1].barh(df["Category"], df[series], left=bottom, label=series, color=color)
    bottom += df[series].values
...
for series, color in zip(
    ...
    axes[1, 0].bar(df["Category"], df_norm[series], bottom=bottom, label=series, color=color)
...
axes[1, 1].bar(
    diverging_data["Category"],
    diverging_data["Positive"],
    color="green",
    alpha=0.7,
    label="Positive",
)
axes[1, 1].bar(
    diverging_data["Category"], diverging_data["Negative"], color="red", alpha=0.7, label="Negative"
)
...
for cat, val, pat in zip(categories, values, patterns):
    axes[0, 1].bar(cat, val, color="white", edgecolor="black", hatch=pat, linewidth=1.5)
...
colors_wf = [
    "gray" if t == "total" else "green" if v > 0 else "red"
...
for i, (cat, val, typ, bot) in enumerate(
    zip(waterfall_data["Category"], waterfall_data["Value"], waterfall_data["Type"], bottoms)
):
    if typ == "total":
