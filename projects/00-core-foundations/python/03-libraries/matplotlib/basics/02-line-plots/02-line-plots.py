...
for i, pattern in enumerate(dash_patterns):
    axes[1, 1].plot(x, np.sin(x) + i * 0.5, linestyle=(0, pattern), linewidth=2, label=f"{pattern}")
...
# Basic error bars
axes[0, 0].errorbar(
    x_err, y_err, yerr=y_err_std, fmt="o-", capsize=5, label="Data ± Error", color="blue"
)
...
axes[0, 1].errorbar(
    x_err, y_err, yerr=[y_err_low, y_err_high], fmt="s-", capsize=5, label="Asymmetric", color="red"
)
...
axes[1, 0].errorbar(
    x_err, y_err, xerr=x_err_vals, yerr=y_err_std, fmt="o", capsize=5, color="green"
)
...
for (x_vals, y_vals, err_vals), color in zip(series, colors):
    axes[1, 1].errorbar(
        x_vals, y_vals, yerr=err_vals, fmt="o-", capsize=5, label=f"Series {color}", color=color
    )
