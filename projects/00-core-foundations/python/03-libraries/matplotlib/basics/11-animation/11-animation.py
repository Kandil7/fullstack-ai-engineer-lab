...
ani = animation.FuncAnimation(fig, animate_multi, frames=200, interval=30, blit=True)
...
ani = animation.FuncAnimation(fig, animate_scatter, frames=200, interval=40, blit=True)
...
def animate_3d(frame):
    ...
    surf = ax.plot_surface(X, Y, Z, cmap="viridis", alpha=0.8, rstride=1, cstride=1, linewidth=0)
    ax.set_zlim(-1.5, 1.5)
...
ani = animation.FuncAnimation(fig, animate_3d, frames=100, interval=50, blit=False)
...
ani = animation.FuncAnimation(fig, animate_particles, frames=200, interval=30, blit=True)
