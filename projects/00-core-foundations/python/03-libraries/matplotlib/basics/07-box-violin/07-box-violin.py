...
bp = axes[1, 1].boxplot(data_list, labels=labels, patch_artist=True, notch=True)
for patch, color in zip(bp["boxes"], colors):
    patch.set_facecolor(color)
...
bp = axes[0, 0].boxplot(
    data_list, labels=labels, patch_artist=True, whis=(5, 95)
...
bp = axes[1, 1].boxplot(data_list, labels=labels, patch_artist=True, showfliers=False)
for patch, color in zip(bp["boxes"], colors):
    patch.set_facecolor(color)
    patch.set_alpha(0.5)
...
# Basic violin plot
parts = axes[0, 0].violinplot(data_list, showmeans=True, showmedians=True, showextrema=True)
...
parts = axes[0, 1].violinplot(
    data_list, showmeans=True, showmedians=True, showextrema=True, vert=False
)
for pc, color in zip(parts["bodies"], colors):
    pc.set_facecolor(color)
    pc.set_alpha(0.7)
...
parts = axes[1, 0].violinplot(
    data_list, showmeans=True, showmedians=True, showextrema=True, bw_method=0.2
)
for pc, color in zip(parts["bodies"], colors):
    pc.set_facecolor(color)
    pc.set_alpha(0.7)
...
parts = axes[1, 1].violinplot(data_list, showmeans=False, showmedians=False, showextrema=False)
for pc, color in zip(parts["bodies"], colors):
    pc.set_facecolor(color)
    pc.set_alpha(0.3)

# Add box plot inside
bp = axes[1, 1].boxplot(data_list, labels=labels, patch_artist=True, widths=0.2, showfliers=False)
for patch, color in zip(bp["boxes"], colors):
...
bp = axes[0, 0].boxplot(data_list, labels=labels, patch_artist=True, showfliers=False)
for patch, color in zip(bp["boxes"], colors):
    patch.set_facecolor(color)
    patch.set_alpha(0.7)
...
for i, data in enumerate(data_list):
    ...
    axes[1, 0].hlines(
        [q1, q3], i + 0.8, i + 1.2, colors="black", linewidth=1.5, linestyles="dashed"
    )
    axes[1, 0].hlines(np.median(data), i + 0.8, i + 1.2, colors="red", linewidth=2)
axes[1, 0].set_xticks(range(1, len(labels) + 1))
axes[1, 0].set_xticklabels(labels)
...
# Grouped violin
parts = axes[0, 1].violinplot(data_by_group, positions=positions, showmeans=True, showmedians=True)
...
for i, t in enumerate(treatments):
    ...
    parts_m = axes[1, 0].violinplot(
        [male_data], positions=[base_pos - 0.15], vert=True, widths=0.3, showextrema=False
    )
    parts_f = axes[1, 0].violinplot(
        [female_data], positions=[base_pos + 0.15], vert=True, widths=0.3, showextrema=False
    )
...
axes[1, 0].legend(
    [plt.Rectangle((0, 0), 1, 1, fc="lightblue"), plt.Rectangle((0, 0), 1, 1, fc="lightpink")],
    ["Male", "Female"],
)
...
for i, (t, g) in enumerate([(t, g) for t in treatments for g in genders]):
    ...
    parts = axes[1, 1].violinplot(
        [data], positions=[pos], widths=0.5, showmeans=False, showmedians=False, showextrema=False
    )
    ...
    # Box
    bp = axes[1, 1].boxplot(
        [data], positions=[pos], widths=0.15, patch_artist=True, showfliers=False
    )
