...
# Step histogram
axes[1, 1].hist(
    normal_data, bins=50, density=True, histtype="step", linewidth=2, color="darkred", label="Step"
)
axes[1, 1].hist(
    normal_data,
    bins=50,
...
# Auto bins (Sturges)
axes[0, 0].hist(data, bins="auto", density=True, alpha=0.7, color="skyblue", edgecolor="black")
...
axes[1, 0].hist(data, bins=bins_fd, density=True, alpha=0.7, color="lightgreen", edgecolor="black")
axes[1, 0].set_title(f"Freedman-Diaconis ({bins_fd} bins)")

# Custom bins
custom_bins = np.linspace(-4, 4, 21)
axes[1, 1].hist(data, bins=custom_bins, density=True, alpha=0.7, color="gold", edgecolor="black")
...
ax_histy.hist(y, bins=30, density=True, alpha=0.5, color="coral", orientation="horizontal")
ax_histx.axis("off")
ax_histy.axis("off")
...
# Histogram with KDE
axes[0, 0].hist(
    data, bins=30, density=True, alpha=0.5, color="skyblue", edgecolor="black", label="Histogram"
)
# KDE
...
axes[0, 0].plot(
    x_grid, stats.norm.pdf(x_grid, data.mean(), data.std()), "g--", linewidth=2, label="Normal Fit"
)
axes[0, 0].legend()
...
parts = axes[1, 1].violinplot(data_list, showmeans=True, showmedians=True, showextrema=True)
