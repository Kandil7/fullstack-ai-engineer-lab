...
```python
result = df.assign(
    profit=lambda x: x["revenue"] - x["cost"], margin=lambda x: x["profit"] / x["revenue"]
)
```
...
```python
df["age_group"] = pd.cut(
    df["age"], bins=[0, 18, 35, 60, 100], labels=["Minor", "Young", "Middle", "Senior"]
)
```
...
```python
df = pd.DataFrame(
    {"date": pd.date_range("2024-01-01", periods=30), "sales": np.random.randint(50, 200, 30)}
)
