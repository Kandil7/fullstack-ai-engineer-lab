...
merged = pd.merge(products, product_sales, on="product_id", suffixes=("_catalog", "_sales"))
print(merged)
...
departments = pd.DataFrame({"dept_id": [1, 2, 3], "dept_name": ["Engineering", "Sales", "HR"]})
...
salaries = pd.DataFrame({"emp_id": [101, 102, 103, 104], "salary": [120000, 110000, 95000, 90000]})

# Cascade: departments → employees → salaries
result = employees.merge(departments, on="dept_id", how="left").merge(
