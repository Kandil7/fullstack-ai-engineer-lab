...
df["churned"] = (df["is_active"] == 0).astype(int) | (
    (df["last_login_days"] > 90) & (np.random.random(n) < 0.3)
).astype(int)
...
for i in range(5):
    df[f"noise_feature_{i}"] = df["income"] * np.random.randn(len(df)) * 0.1 + np.random.randn(
        len(df)
    )
...
def create_features(df):
    ...
    df["income_bracket"] = pd.qcut(
        df["income"], q=5, labels=["VeryLow", "Low", "Medium", "High", "VeryHigh"]
    )
