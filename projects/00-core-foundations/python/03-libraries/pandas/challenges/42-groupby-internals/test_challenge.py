...
class TestManualGroupMean:
    ...
    def test_numeric_columns_only(self) -> None:
        df = pd.DataFrame({"team": ["a", "b"], "x": [1.0, 2.0], "name": ["alice", "bob"]})
        result = mod.manual_group_mean(df, "team")
        assert result.columns.tolist() == ["x"]

    def test_empty_frame(self) -> None:
        df = pd.DataFrame({"team": pd.Series(dtype="str"), "x": pd.Series(dtype="float64")})
        result = mod.manual_group_mean(df, "team")
        assert len(result) == 0
...
class TestGroupMetrics:
    ...
    def test_mean_skips_nan(self) -> None:
        df = pd.DataFrame({"team": ["a", "a", "a"], "x": [1.0, 2.0, np.nan]})
        result = mod.group_metrics(df, "team")
...
class TestCohortRetention:
    ...
    def test_diagonal_for_single_month_users(self) -> None:
        ...
        df = pd.DataFrame({"user_id": users, "month": ["2024-01"] * 5})
        result = mod.cohort_retention(df)
        assert result.shape == (1, 1)
        assert result.iloc[0, 0] == 1.0

    def test_empty(self) -> None:
        df = pd.DataFrame({"user_id": pd.Series(dtype="int64"), "month": pd.Series(dtype="str")})
        result = mod.cohort_retention(df)
        assert len(result) == 0
    ...
    def test_column_zero_always_one(self) -> None:
        ...
        result = mod.cohort_retention(df)
        assert bool((result[0] == 1.0).all()), "month 0 retention is 1.0 by construction"
