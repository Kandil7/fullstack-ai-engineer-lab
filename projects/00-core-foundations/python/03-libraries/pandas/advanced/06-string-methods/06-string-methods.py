...
df = pd.DataFrame(
    {
        ...
        "phone": [
            f"+1-555-{np.random.randint(100, 999)}-{np.random.randint(1000, 9999):04d}"
            for _ in range(20)
        ],
        "address": [
            f"{np.random.randint(100, 9999)} Main St, City {i}, ST {np.random.randint(10000, 99999)}"
            for i in range(1, 21)
        ],
        "tags": [
            ",".join(
                np.random.choice(
                    ["python", "ml", "data", "ai", "web", "cloud", "devops", "sql"],
        ...
        "description": [
            f"This is a sample description for user {i} with some numbers {np.random.randint(100, 999)}"
            for i in range(1, 21)
        ],
        "code": [
            f"CODE-{np.random.randint(1000, 9999)}-{np.random.choice(['A', 'B', 'C'])}"
            for _ in range(20)
        ],
    }
)
...
# Simple replace
print(
    "Replace 'User' -> 'Customer':", df["full_name"].str.replace("User", "Customer").head().tolist()
)
print()
...
print(
    ...
    df["description"].str.replace(r"\d+", "XXX", regex=True).head().tolist(),
)
print()
...
addr_parsed = df["address"].str.extract(
    r"(?P<street_num>\d+)\s+(?P<street>\w+\s\w+),\s+(?P<city>\w+\s\d+),\s+(?P<state>\w+)\s+(?P<zip>\d+)"
)
...
df["slug"] = (
    df["full_name"].str.lower().str.replace(" ", "-").str.replace(r"[^\w-]", "", regex=True)
)
print("Slugs:")
