...
# Clean boolean
df["is_active_clean"] = df["is_active"].map(
...
df_best = df_filled.sort_values("completeness", ascending=False).drop_duplicates(
    subset=["customer_id_clean"], keep="first"
)
...
outliers_iqr = df_best[
    (df_best["income_clean"] < lower_bound) | (df_best["income_clean"] > upper_bound)
]
...
# Z-score method
z_scores = np.abs(
    (df_best["income_clean"] - df_best["income_clean"].mean()) / df_best["income_clean"].std()
)
...
print(
    f"Capped income range: [{df_capped['income_capped'].min()}, {df_capped['income_capped'].max()}]"
)
print()
