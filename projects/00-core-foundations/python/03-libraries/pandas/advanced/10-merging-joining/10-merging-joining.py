...
employees = pd.DataFrame(
    {
        ...
        "hire_date": pd.to_datetime(
            ["2020-01-15", "2019-03-22", "2021-07-10", "2022-02-01", "2018-11-30", "2023-05-15"]
        ),
    }
)
print("Employees:")
...
joined_multi = emp_indexed.join(dept_indexed, how="left", rsuffix="_dept").join(
    projects_indexed, how="left", rsuffix="_proj"
)
...
df_main = pd.DataFrame({"A": [1, np.nan, 3], "B": [np.nan, 5, np.nan], "C": [7, 8, 9]})
df_fill = pd.DataFrame({"A": [10, 20, 30], "B": [40, 50, 60], "D": [70, 80, 90]})
...
current.loc[current["customer_id"] == 1, ["valid_to", "is_current"]] = [
...
org_self = pd.merge(
    org, org, left_on="manager_id", right_on="emp_id", how="left", suffixes=("", "_mgr")
)
...
n = 100000
df_large1 = pd.DataFrame({"key": np.random.randint(0, 10000, n), "value1": np.random.randn(n)})
df_large2 = pd.DataFrame({"key": np.random.randint(0, 10000, n), "value2": np.random.randn(n)})
