...
melted = pd.melt(wide_df, id_vars=["employee"], var_name="metric_month", value_name="value")
...
pivot_simple = pd.pivot_table(
    df_unique, index="region", columns="product", values="revenue", aggfunc="sum"
)
...
# wide_to_long
long_numbered = pd.wide_to_long(wide_numbered, stubnames=["X", "Y"], i="id", j="time")
...
monthly_trend = pd.pivot_table(
    df, values="revenue", index="month", columns="product", aggfunc="sum", fill_value=0
)
...
rep_perf = pd.crosstab(
    df["sales_rep"], df["product"], values=df["revenue"], aggfunc="sum", margins=True
)
...
cohort_pivot = pd.pivot_table(
    cohort_long, values="revenue", index="signup_month", columns="period_num", aggfunc="mean"
)
...
customer_product = pd.crosstab(
    df["sales_rep"], df["product"], values=df["revenue"], aggfunc="sum"
).fillna(0)
