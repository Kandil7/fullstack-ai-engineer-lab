...
# Horizontal bar
ax = channel_sales.plot(
    kind="barh", figsize=(8, 5), title="Avg Sales by Channel (Horizontal)", color="teal"
)
...
# Histogram
ax = df["sales"].plot(
    kind="hist", bins=20, figsize=(8, 5), title="Sales Distribution", alpha=0.7, edgecolor="black"
)
ax.set_xlabel("Sales ($)")
plt.tight_layout()
...
large_df = pd.DataFrame({"x": np.random.randn(5000), "y": np.random.randn(5000)})
ax = large_df.plot(
    kind="hexbin", x="x", y="y", gridsize=30, figsize=(8, 6), title="Hexbin Plot", colormap="Blues"
)
plt.tight_layout()
...
ax = device_counts.plot(
    kind="pie", figsize=(6, 6), title="Device Distribution", autopct="%1.1f%%", startangle=90
)
plt.ylabel("")
plt.tight_layout()
...
# Area plot
df.set_index("date")[["sales", "visitors"]].plot.area(
    figsize=(10, 5), title="Cumulative Sales & Visitors", alpha=0.5
)
plt.tight_layout()
...
df.groupby("region")["sales"].sum().plot(
    kind="pie", ax=axes[0, 1], title="Sales by Region", autopct="%1.1f%%"
)
df.groupby("device")["conversion_rate"].mean().plot(
    kind="barh", ax=axes[1, 0], title="Conversion by Device"
)
...
df.set_index("date")["sales"].plot(
    ax=ax, color="#2E86AB", linewidth=2, alpha=0.8, label="Daily Sales"
)

# Add rolling average
df.set_index("date")["sales"].rolling(7).mean().plot(
    ax=ax, color="#A23B72", linewidth=3, label="7-Day MA"
)
...
if not HAS_SEABORN:
    ...
    sns.pairplot(
        df[["sales", "visitors", "conversion_rate", "channel"]].sample(200, replace=True),
        hue="channel",
        diag_kind="kde",
        corner=True,
    )
    ...
    sns.heatmap(
        corr, annot=True, cmap="RdBu_r", center=0, ax=ax, square=True, cbar_kws={"shrink": 0.8}
    )
...
df["channel"] = pd.Categorical(
    df["channel"], categories=["Organic", "Paid", "Social", "Email"], ordered=True
)
df["device"] = pd.Categorical(
    df["device"], categories=["Desktop", "Tablet", "Mobile"], ordered=True
)

# Ordered bar plot respects category order
fig, axes = plt.subplots(1, 2, figsize=(12, 5))
df.groupby("channel")["sales"].mean().plot(
    kind="bar", ax=axes[0], title="Ordered by Channel Priority"
)
df.groupby("device")["conversion_rate"].mean().plot(
    kind="bar", ax=axes[1], title="Ordered by Device Size"
)
plt.tight_layout()
