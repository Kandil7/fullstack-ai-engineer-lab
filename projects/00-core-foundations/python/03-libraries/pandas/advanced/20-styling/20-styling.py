...
# Bar charts in cells
styled_bar = df.style.bar(
    subset=["Q1_Sales", "Q2_Sales", "Q3_Sales", "Q4_Sales"], color="#5fba7d", align="zero"
)
...
html_output = (
    ...
    .background_gradient(cmap="RdYlGn", subset=["Growth", "Margin"])
    .to_html()
)
...
financial_style = (
    financial.style.format(
        {"Actual": format_currency, "Budget": format_currency, "Variance": "{:.1%}"}
    )
    .applymap(variance_color, subset=["Variance"])
    .bar(subset=["Variance"], color=["#f8d7da", "#d4edda"], align="zero")
)
...
for i, row in financial.iterrows():
    trend = row["Trend"]
    sparkline = "".join(
        ["#" * int((v - min(trend)) / (max(trend) - min(trend)) * 10) for v in trend]
    )
