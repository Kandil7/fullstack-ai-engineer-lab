...
leaky_scaler = StandardScaler().fit(np.concatenate([train_vals, test_vals]).reshape(-1, 1))

print(
    "Correct: train mean",
    round(float(correct_scaler.mean_[0]), 2),
    "| test transformed:",
    correct_scaler.transform(test_vals.reshape(-1, 1)).ravel().round(1).tolist(),
)
print(
    "Leaky:   train mean",
    round(float(leaky_scaler.mean_[0]), 2),
    "| test transformed:",
...
print(
    ...
    len(test_ts),
    "| test starts:",
    test_ts["date"].iloc[0].strftime("%Y-%m-%d"),
)
...
def _verify() -> None:
    ...
    assert "days_since_epoch" in engineered.columns, "engineering must add date-derived features"
    ...
    assert "plan_enterprise" in test_aligned.columns, "reindex must restore the full column set"
    assert int(test_aligned["plan_enterprise"].sum()) == 0, "missing category must be filled with 0"

    # Leakage: scaler fit on train only has the TRAIN mean.
    assert correct_scaler.mean_[0] == 2.0, "scaler fitted on train must have the train mean"
    ...
    assert (train_ts["date"] < cutoff).all(), "chronological train must precede the cutoff"
    assert (test_ts["date"] >= cutoff).all(), "chronological test must start at the cutoff"
    ...
    assert transformed.shape == (20, 3), "1 scaled column + 2 one-hot columns (drop='first')"
