...
print("broadcast_to is a view (base is not None):", view_only.base is not None)
...
def try_broadcast(shp_a: tuple[int, ...], shp_b: tuple[int, ...]) -> str:
...
def _verify() -> None:
    ...
    assert (np.ones((3, 4)) + np.ones(4)).shape == (3, 4), "(4,) must align with the trailing axis"
    ...
    assert np.broadcast_to(a6[:, None], (5, 4)).base is not None, "broadcast_to must not copy"
    ...
    assert oh[1, 2] == 1.0 and oh[1, 0] == 0.0, "label 2 must land in column 2"
