...
def minimize_box(func, x0: float, lo: float, hi: float) -> float:
    ...
    res = optimize.minimize(func, np.array([x0]), method="L-BFGS-B", bounds=[(lo, hi)])
    if not res.success:
...
def fit_robust_line(x: np.ndarray, y: np.ndarray, loss: str) -> tuple[float, float]:
    ...
    res = optimize.least_squares(lambda p: p[0] * x + p[1] - y, x0=np.zeros(2), loss=loss)
...
def allocate_weights(mu: np.ndarray, cov: np.ndarray, risk_free: float) -> np.ndarray:
    ...
    res = optimize.minimize(
        neg_sharpe, np.full(n, 1.0 / n), method="SLSQP", bounds=[(0.0, 1.0)] * n, constraints=cons
    )
    if not res.success:
        raise RuntimeError(f"optimizer failed: {res.message}")
    return res.x
