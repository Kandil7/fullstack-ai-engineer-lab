"""
Design Patterns - Advanced Python Exercises
=============================================
...
class EventSystem(Subject):
    ...
    def emit(self, event: str, data: Any = None):
        if event not in self.events:
            self.events[event] = []
        self.events[event].append({"data": data, "timestamp": __import__("time").time()})
...
class AlertObserver(Observer):
    ...
    def __init__(self, alert_events: List[str]):
...
class USAdapter:
    """Adapter for American socket to work with European interface."""
...
class Laptop:
    """Laptop that needs European socket."""
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 60)
    print("All design pattern demos complete!")
    print("=" * 60)
