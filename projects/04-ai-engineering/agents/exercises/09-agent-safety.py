...
class ToolAccessController:
    ...
    def check_permission(
        ...
        if (
            permission.max_invocations
            and permission.invocation_count[agent_id] >= permission.max_invocations
        ):
            violation = SafetyViolation(
                violation_id=str(uuid.uuid4())[:8],
                threat_level=ThreatLevel.HIGH,
...
class RateLimiter:
    ...
    def __init__(self):
        ...
        self.global_limit = RateLimitConfig(
            max_requests=1000, window_seconds=3600, max_tokens=1000000, max_cost=100.0
        )
        self.global_usage = {"requests": [], "tokens": 0, "cost": 0.0}

    def create_bucket(
        self, bucket_id: str, max_tokens: int, refill_rate: float
    ) -> None:
    ...
    def acquire(self, bucket_id: str, tokens: int = 1) -> Tuple[bool, Dict[str, Any]]:
        """
        ...
        if bucket["tokens"] >= tokens:
            ...
            return True, {"remaining": bucket["tokens"], "max": bucket["max_tokens"]}
    ...
    def check_global_limit(self, tokens: int, cost: float) -> Tuple[bool, str]:
        ...
        self.global_usage["requests"] = [
            t
            for t in self.global_usage["requests"]
...
class AuditLogger:
    ...
    def log_safety_violation(self, violation: SafetyViolation) -> AuditEntry:
...
class KillSwitch:
    ...
    def register_agent(self, agent_id: str, shutdown_fn: Callable) -> None:
    ...
    async def activate(self, reason: str, immediate: bool = True) -> Dict[str, Any]:
...
class AgentSafetySystem:
    ...
    def get_safety_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "violations_by_level": {
                level.value: sum(1 for v in self.violations if v.threat_level == level)
...
async def main():
    ...
    system.tool_controller.register_tool(
        "web_search", allowed_agents={"agent_1", "agent_2"}, rate_limit=10
    )
    system.tool_controller.register_tool(
        "execute_code", allowed_agents={"agent_1"}, rate_limit=2, requires_approval=True
    )
