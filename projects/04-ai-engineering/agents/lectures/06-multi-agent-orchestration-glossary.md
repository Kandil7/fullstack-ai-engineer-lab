...
class Pipeline:
    ...
    def add_step(self, name: str, processor: Callable, config: dict = None):
        ...
        self.steps.append(
            {"name": name, "processor": processor, "config": config or {}}
        )
        return self
...
RESEARCHER = AgentRole(
    ...
    responsibilities=["Find relevant information", "Verify facts", "Provide sources"],
)
...
code_agent = SpecializedAgent(
    "CodeBot", "programming", ["python", "javascript", "debugging"]
)
research_agent = SpecializedAgent(
    "ResearchBot", "research", ["search", "analysis", "citation"]
)
