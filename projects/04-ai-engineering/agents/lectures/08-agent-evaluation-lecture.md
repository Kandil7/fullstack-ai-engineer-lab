...
class AgentEvaluator:
    ...
    def run_test(self, test_case: TestCase) -> TestResult:
        ...
        try:
            ...
            # Evaluate output
            metrics = self._evaluate_output(test_case, actual_output, execution_time)
    ...
    def _evaluate_output(
        self, test_case: TestCase, actual: Any, execution_time: float
    ) -> Dict:
    ...
    def _determine_status(
        self, test_case: TestCase, actual: Any, metrics: Dict
    ) -> TestStatus:
    ...
    def compare_agents(
        self, agents: Dict[str, Callable], test_cases: List[TestCase] = None
    ) -> Dict:
...
class LLMJudge:
    ...
    def evaluate(self, task: str, output: str, criteria: List[str] = None) -> Dict:
        """
    ...
    def batch_evaluate(self, evaluations: List[Dict]) -> List[Dict]:
        ...
        for eval_item in evaluations:
            result = self.evaluate(
                eval_item["task"], eval_item["output"], eval_item.get("criteria")
            )
...
evaluator.add_test_case(
    TestCase(
        ...
        expected_behavior={"contains_words": ["machine", "learning"], "min_length": 20},
        tags=["basic", "content"],
    )
)
...
evaluation = judge.evaluate(task="What is AI?", output="AI is artificial intelligence.")
