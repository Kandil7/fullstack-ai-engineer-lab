...
class AgentMetrics:
    ...
    def record_request(self, success: bool, latency: float):
        ...
        self.request_history.append(
            {"success": success, "latency": latency, "timestamp": time.time()}
        )
...
class ProductionAgent:
    ...
    def __init__(self, name: str, core_agent: Callable, config: Dict = None):
        self.name = name
    ...
    def process(self, input_data: Any, request_id: str = None) -> Dict:
        """
        ...
        try:
            ...
            # Log error
            self.logger.error(f"Request failed: {request_id}", exc_info=True)

            trace["steps"].append(
                {"step": "error", "error": str(e), "time": time.time()}
            )
...
class AgentMonitor:
    ...
    def check_all_agents(self) -> Dict:
        ...
        for name, agent in self.agents.items():
            ...
            results[name] = {"health": health, "metrics": metrics}
    ...
    def _check_alerts(self, agent_name: str, health: Dict, metrics: Dict):
...
for i in range(5):
    ...
    print(
        f"Request {i}: {'Success' if result['success'] else 'Failed'} "
...
@app.route("/health")
def health_check():
    return {"status": agent.health_status.value, "metrics": agent.get_metrics()}
```
