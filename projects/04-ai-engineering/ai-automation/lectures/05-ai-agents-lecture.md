...
"""
        ...
        # Get LLM response
        response = self.llm.generate(system=system_prompt, messages=self.memory)
    ...
    def _parse_action(self, response):
        ...
        return {"type": "action", "thought": thought, "tool": tool, "input": tool_input}
```
...
"""
    ...
    def _parse_action(self, response: str) -> Action:
        ...
        if "Final Answer:" in response:
            ...
            return Action(type=ActionType.FINISH, thought=thought, result=answer)
        ...
        if "Action:" in response:
            try:
                ...
                return Action(
                    type=ActionType.ERROR, result=f"Failed to parse action: {str(e)}"
                )

        # Default: treat as finish
        return Action(type=ActionType.FINISH, result=response)
...
def create_research_agent():
    ...
    # Create agent
    agent = AIAgent(
        name="Research Assistant", model="gpt-4", tools=tools, max_iterations=10
    )
