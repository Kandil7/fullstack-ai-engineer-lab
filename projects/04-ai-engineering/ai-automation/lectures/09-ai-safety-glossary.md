...
# Example
job_posting = (
    "We're looking for a talented developer. He should have 5+ years experience..."
)
...
guardrails.add_rule("max_length", lambda text: len(text) <= 1000, "block")

# Add content filter
guardrails.add_rule("no_harmful", lambda text: "harm" not in text.lower(), "block")
...
def detect_hallucination_risk(text, context=None):
    ...
    uncertainty_count = sum(
        1 for phrase in uncertainty_phrases if phrase in text.lower()
    )
...
class RedTeamTest:
    ...
    def add_test(self, name, input_text, expected_behavior):
        ...
        self.test_cases.append(
            {"name": name, "input": input_text, "expected": expected_behavior}
        )
    ...
    def run_tests(self, system):
        ...
        for test in self.test_cases:
            try:
                ...
                results.append(
                    {"test": test["name"], "passed": passed, "output": output[:100]}
                )
...
def detect_adversarial(text):
    ...
    return {"is_adversarial": len(findings) > 0, "attack_types": findings}
```
