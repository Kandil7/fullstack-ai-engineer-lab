...
class ContentModerator:
    ...
    def moderate(self, text: str) -> ModerationResult:
        ...
        # Default to safe
        return ModerationResult(
            category=ContentCategory.SAFE, confidence=0.8, flags=[], action="allow"
        )
...
class Guardrail:
    ...
    def check(self, content: Any) -> GuardrailResult:
        ...
        if passed:
            return GuardrailResult(action=GuardrailAction.PASS, message="Passed")
        else:
            return GuardrailResult(action=self.action, message=message)
...
{{
    ...
    def safe_generate(self, prompt: str, **kwargs) -> Dict[str, Any]:
        ...
        return {"success": True, "output": output, "warnings": [], "filtered": False}
    ...
    def get_audit_report(self) -> Dict:
        ...
        return {
            ...
            "categories": list(set(check["type"] for check in self.audit_log)),
